...
test("select where returns results", async () => {
	...
	const results = await database.query(TestTableAllTypes.select("*", ["id", "some_string"])([1, "hello world"]))

	expect(results).toEqual([{ ...dataset1, some_date: moment(date1).format("YYYY-MM-DD"), id: 1 }])
})
...
test("select where is not null", async () => {
	...
	expect(results).toEqual([{ ...dataset3, some_date: moment(date2).format("YYYY-MM-DD"), id: 3 }])
})
