...
describe('init', () => {
    ...
    test('initialize twice without restart fails', async () => {
        ...
        await expect(databaseSchema.init())
            .rejects.toThrowError(`Database schema ${databaseSchema.name} has already been initialized.`)
    })
})
...
describe('migrate latest', () => {
    test('valid migrations succeed', async () => {
        ...
        migrations.set(2, Migration(async (transaction) => {
            ...
            await transaction.query(TestTableA.addColumns({
                some_newly_added_col_string: { type: ColumnType.Varchar, nullable: false, defaultValue: 'Hello World' },
    ...
    test('no schema initialization fails', async () => {
        ...
        await expect(databaseSchema.migrateLatest())
            .rejects.toThrowError('Migration failed, database schema is not initialized. Please call init() first on your database schema.')
    ...
    test('sql error aborts migration', async () => {
        ...
        await expect(databaseSchema.migrateLatest())
            .rejects.toThrowError('cannot drop table test_table_a column id because other objects depend on it')
        expect(databaseSchema.getVersion()).toBe(1)
    })
    ...
    test('missing migration aborts', async () => {
        ...
        migrations.set(4, Migration(async (transaction) => undefined))
        ...
        await expect(databaseSchema.migrateLatest())
            .rejects.toThrowError(`Migration with version 3 not found. Aborting migration process...`)
...
describe('migrate to version', () => {
    ...
    test('migrate to version lower 2 fails', async () => {
        ...
        await databaseSchema.init()
        await expect(databaseSchema.migrateToVersion(1))
            .rejects.toThrowError('Target version of migrateToVersion() has to be greater 1')
        await expect(databaseSchema.migrateToVersion(0))
            .rejects.toThrowError('Target version of migrateToVersion() has to be greater 1')
        await expect(databaseSchema.migrateToVersion(-5))
            .rejects.toThrowError('Target version of migrateToVersion() has to be greater 1')
    })
})